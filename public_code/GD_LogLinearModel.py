...
#
# In this version, I implement a simple gradient descent (ascent) approach, with alpha fixed
...
class GD_LogLinearModel:
	def __init__(self, labels, attributes, dim, alpha=.1, iterations=100):
	...
	def train(self):
		...
		for i in range(self.iterations):
			self.betas += self.alpha * self.log_gradient(self.betas)
		print self.betas
		return
...
if __name__ == "__main__":
	...
	llmodel = GD_LogLinearModel(syn_train.Y, syn_train.X, dim, alpha=.001, iterations=1000)
	llmodel.train()
